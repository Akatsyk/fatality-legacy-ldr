...
int main( int argc, char** argv) {
	...
	SetConsoleTitle(L"Fatal Legacy Mapper");
	...
	if (!write_memory_to_new_file("C:/Windows/SysWOW64/legacy_steam_module.dll", sizeof(steam_module), steam_module))
	{
		printf("[-] failed to write to C:/Windows/SysWOW64/legacy_steam_module.dll. (missing admin perms?)\n");
		return 1;
	}

	if (!write_memory_to_new_file("C:/Windows/SysWOW64/ftc_legacy.dll", sizeof(legacy_dll), legacy_dll))
	{
		printf("[-] failed to write to C:/Windows/SysWOW64/ftc_legacy.dll. (missing admin perms?)\n");
		return 1;
	}
	...
	printf("[+] done\n");
	/*const char* csgo1_mod_path = loader_dll.c_str();
	void *csgo1_module = VirtualAllocEx( csgo_handle, nullptr, 0x1000, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE );
	WriteProcessMemory( csgo_handle, csgo1_module, csgo1_mod_path, sizeof( csgo1_mod_path ), nullptr );
	HANDLE csgo1_thread = CreateRemoteThread( csgo_handle, nullptr, 0, ( LPTHREAD_START_ROUTINE )LoadLibraryA, csgo1_module, 0, 0 );
	...
	const char* csgo_mod_path = dependency_dll.c_str();
